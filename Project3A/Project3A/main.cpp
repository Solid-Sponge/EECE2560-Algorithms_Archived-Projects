...
/*!
* \param wordlist& wordlist passed
* \param grid& grid of chars passed
*/
void findMatches(wordlist&, grid&);

//! Given an UNSORTED or SORTED wordlist and a grid of chars, finds all possible words. Uses a hash table lookup on a dictionary
/*!
* \param wordlist& SORTED wordlist passed
* \param grid& grid of chars passed
*/
template <typename T>
void findMatchesHash(wordlist&, grid&);
...
/*!
* \param int Sorting algorithm used. 0 = InsertionSort (30 min)  1 = QuickSort (2.418 sec)  2 = MergeSort (10.362 sec)   3 = HeapSort (5.007 sec)    4 = Hash Table Lookup
*/
void search(int);
...
void findMatches(wordlist& dictionary, grid& file)
{
	...
	for (int i = 0; i < size; i++) //!< Goes through each column of the array
	{
		for (int j = 0; j < size; j++) //!< Goes throw each row of the array
		{
			...
			for (int k = 0; k < size && k < 15; k++) //!< Goes through each possible combination of words in a line until either the dimension of the matrix is reached or k > 12
...
{
	if (searchType != 0 && searchType != 1 && searchType != 2 && searchType != 3 && searchType != 4)
	{
		throw std::invalid_argument("INVALID VALUE: searchType must be equal to 0, 1, 2, 3, or 4");
	}
		wordlist dictionary;
	...
	case 3: //! Heap Sort
		...
		dictionary.HeapSort();
		sorttime = double(clock() - startTime) / CLOCKS_PER_SEC;
		std::cout << "HEAP SORT: " << sorttime << " Seconds" << std::endl;
		break;
	case 4: //! Hash Table Lookup
		break;
	}
	startTime = clock();
	if (searchType != 4)
	...
	{
		findMatchesHash<string>(dictionary, file); //!< Hash Table Lookup
		searchtime = double(clock() - startTime) / CLOCKS_PER_SEC;
		std::cout << "Time take to find words in grid (Time Overall): " << searchtime << " Seconds" << std::endl;
	}
}
...
template <typename T>
void findMatchesHash(wordlist &dictionary,grid& file)
{
	hashtable<string> table(89000);
	for (int i = 0; i < dictionary.returnSizeOfWords(); i++)
	{
		table.addItem(dictionary.returnList()[i]);
	}
	int size = file.getSize();
	matrix<char>* charMatrix = &(file.getMatrix());
			...
			{										 //!< as there are very few words in the english alphabet that are greater than 11 words, this speeds up the algorithm
				...
				if (x >= 4) //!Checks if test words are greater than 4 letters
				{
					...
					for (int direction = 0; direction < 8; direction++) //!< Tests each element in the test vector of strings against the dictionary via Binary search.
					{
						...
						//if (dictionary.BinarySearch(dictionary.returnList(), testWord[direction], 0, dictionary.returnSizeOfWords() - 1) != -1 && testWord[direction].length() > 4)
						if (testWord[direction].length() > 4 && table.inList(testWord[direction]))
						{
							WordsFound.push_back(testWord[direction]); //!< If word found, pushes it onto WordsFound vector
						}
						testWord[direction] = ""; //!< Resets test vector of strings
