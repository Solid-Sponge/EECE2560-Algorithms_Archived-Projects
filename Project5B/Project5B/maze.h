...
// Create a graph g that represents the legal moves in the maze m.
{
	...
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
		{
			...
			int index = i * rows + j; //Index of current vertex placed
			...
			if (above >= 0)
				indextop = above * rows + j;
			if (left >= 0)
				indexleft = i * rows + left;
			if (above >= 0 && left >= 0)
				indextopleft = above * rows + left;
...
void maze::printPath(Graph::vertex_descriptor end,
	...
	Graph g)
{
	stack<Graph::vertex_descriptor> reversedS;
	while (!s.empty())
	{
		reversedS.push(s.top());
		s.pop();
	}
	
	while (!reversedS.empty())
	{
		Graph::vertex_descriptor vItr = reversedS.top();
		reversedS.pop();
